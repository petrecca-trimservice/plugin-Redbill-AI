<?php
/**
 * MSG Parser — OLE Compound Document
 *
 * Parsa file .msg (formato OLE/Compound Document) ed estrae allegati.
 * Classe mantenuta con il nome originale per compatibilità con il frontend.
 *
 * @package Redbill_AI
 * @since   1.0.0
 */
...
class MSG_Universal_Parser_V7 {
    ...
    /** @var array[] Allegati estratti: [['name' => string, 'data' => string], ...] */
    public $attachments = [];
    ...
    private function parse(): void {
        ...
        if ( strlen( $raw_fat ) > 0 ) {
            $this->fat = array_values( unpack( 'V*', $raw_fat ) );
        }

        $dir_stream  = $this->readChain( $dir_start_sect );
        $num_entries = (int) ( strlen( $dir_stream ) / 128 );
        ...
        if ( isset( $this->directory[0] ) ) {
            ...
            if ( strlen( $raw_minifat ) > 0 ) {
                $this->miniFat = array_values( unpack( 'V*', $raw_minifat ) );
            }
        }
    ...
    private function readSector( int $sect ): string {
    ...
    private function readChain( int $start_sect ): string {
    ...
    private function readMiniChain( int $start_sect ): string {
    ...
    private function parseDirEntry( string $binary, int $index ): array {
    ...
    private function findAttachments(): void {
        foreach ( $this->directory as $id => $entry ) {
            if ( $entry['type'] === 1 && stripos( $entry['name'], '__attach' ) === 0 ) {
                ...
                if ( $attData ) {
                    $this->attachments[] = $attData;
                }
            }
        }
    }

    private function extractAttachmentFromStorage( array $storageEntry ): ?array {
        ...
        if ( $binaryData && $filename ) {
            return [ 'name' => $filename, 'data' => $binaryData ];
        }
        return null;
    }

    private function getAllChildren( int $rootId ): array {
        $results = [];
        ...
        while ( ! empty( $stack ) ) {
            ...
            if ( isset( $node['leftChild'] ) && $node['leftChild'] !== 0xFFFFFFFF ) {
                $stack[] = $node['leftChild'];
            }
            if ( isset( $node['rightChild'] ) && $node['rightChild'] !== 0xFFFFFFFF ) {
    ...
    private function getStreamData( array $entry ): string {
        if ( $entry['size'] < 4096 ) {
            return substr( $this->readMiniChain( $entry['start_sect'] ), 0, $entry['size'] );
        }
        return substr( $this->readChain( $entry['start_sect'] ), 0, $entry['size'] );
    }
}
